...
@pytest.fixture(scope="module")
def execute_query(sql_server_connection,oracle_connection):
    print("Test")
...
def execute_query_count(sql_server_connection,oracle_connection):
    ...
    print(oracle_conn)
    # query=""
    sql_count=pd.read_sql_query(f"Select count(*) as count from EMP_1",sql_conn)
    oracle_count=pd.read_sql_query(f"Select count(*) as count from EMPLOYEES",oracle_conn)
    print(sql_count)
    print(oracle_count)
